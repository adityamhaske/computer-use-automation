...
Deliberate, in the order they would be restored:

- **Capability catalog** (store, tool-spec export, `cua catalog`) — cut first, as planned. The
  artifact already emits JSON Schema for its typed I/O, so the agent-facing contract exists; what is
  missing is discovery-by-name, which is plumbing rather than design.
- **Evals** (`replay_stability`, `cross_tenant` suites and their reports) — the deterministic
  assertions they would make are already tests. What is genuinely lost is the *aggregate* view:
  drift trend and strategy mix over many runs, which is what `CapabilityEvaluation` is shaped for.

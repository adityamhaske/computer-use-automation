...
**The discovery evidence in this repository is scripted, not live.** Every committed
`run_record.json` under [`evidence/discovery/`](evidence/discovery/) reports
`model_name: "fake/scripted"`. The live path exists and is exercised by
`tests/e2e/test_live_discovery.py` (deselected by default, needs a key), but brief §4 asks for a
genuine model run committed as evidence and this repository does not yet contain one. It is the one
gap here that is not a deliberate cut.
